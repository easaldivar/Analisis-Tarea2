/** Adaptado de: uhr-main/utils.cpp
 *  Autor: Leonardo Lovera */

#ifndef EXPERIMENTO_UTILS
#define EXPERIMENTO_UTILS
...
{
    if (argc != 5) {
        std::cerr << "Usage: <RUNS> <LOWER> <UPPER> <STEP>" << std::endl;
    ...
    try {
        ...
        lower = std::stoll(argv[2]);
        upper = std::stoll(argv[3]);
        step  = std::stoll(argv[4]);
